...
local function get_record_simple(name)
	...
	elseif (not muteEntry.record) or (#muteEntry.record == 0) then
		return nil, ("`%s' has no mute records"):format(name)
	end
	local record = { }
	...
	return record, muteEntry.record
end
...
local function make_fs(name)
	...
	if record_name then
		local record, muteEntry = get_record_simple(record_name)
		if record then
			...
			fsn=fsn+1 fs[fsn] = "textlist[4.2,.8;11.7,9.3;err;"..ESC(muteEntry)..";0]"
			fsn=fsn+1 fs[fsn] = "label[0,10.3;"..ESC(muteEntry).."]"
		end
	else
		local muteEntry = "No entry matches the query."
		fsn=fsn+1 fs[fsn] = "textlist[4.2,.8;11.7,9.3;err;"..ESC(muteEntry)..";0]"
		fsn=fsn+1 fs[fsn] = "label[0,10.3;"..ESC(muteEntry).."]"
	end
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if not minetest.check_player_privs(name, { mute=true }) then
		minetest.log("warning",
				"[mute] Received fields from unauthorized user: "..name)
		return
	end
	local state = get_state(name)
...
minetest.register_chatcommand("mute_gui", {
	description = "Show Mute GUI",
	params = "",
	privs = { mute=true, },
